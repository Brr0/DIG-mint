// Purpose: Contains texts that will be used in the UI when rendering the buttons
// label has to be the exact same as in your candy machine config
export const mintText = [
  //{ label: "wl", mintText: "Mint with a WL token for 480 USDC", buttonLabel: "Mint now!", header: "WL Mint" },
 // { label: "OG", mintText: "Mint Paying with ABC", buttonLabel: "Mint now!", header: "OG Mint" },
  { label: "pub", mintText: "Public mint for 480 USDC", buttonLabel: "Mint now!", header: "Public Mint" },
 // { label: "default", mintText: "Mint with a WL token for 480 USDC", buttonLabel: "Mint now!", header: "WL mint"}
];

//header image in the ui. replace with your own
export const image = "https://ktvuzmyfcg3agyjtjwjrsxdgrwitpwbuwfw4fzvj5jglyr56u6zq.arweave.net/VOtMswURtgNhM02TGVxmjZE32DSxbcLmqepMvEe-p7M"; 

//website title
export const headerText = "DIG Australia";